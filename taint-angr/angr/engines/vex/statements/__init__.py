...
#my add
def taint(stmt,state):
    stmt_str = str(stmt)
    out_str = stmt_str+'--->'
    if 'IMark' in stmt_str:
        pass 
    else:
        for key in state.scratch.temps:
            tmp_value = 't'+str(key)
            if tmp_value in stmt_str:
                out_str = out_str + tmp_value+':'+str(state.scratch.temps[key])+';' 
    state.ir_trace.append(out_str)
    return
