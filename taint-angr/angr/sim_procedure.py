...
class SimProcedure:
    ...
    #my add
    def handle_func(self,inst,func_name):
        argc_ir = len(inst.arguments)
        argv_ir = inst.arguments
        
        arg_str = []
        
        for i in range(argc_ir):
            bv_value = argv_ir[i]
            bv_str = str(bv_value).split(' ')[-1][0:-1]
            bv_int = None
            if '0x' in bv_str:
                bv_int = int(bv_str,16)
            else:
                bv_int = int(bv_str)
            
            bv_find = False
            for key in inst.state.scratch.temps:
                value_str = str(inst.state.scratch.temps[key]).split(' ')[-1][0:-1]
                value_int = None
                if '0x' in value_str:
                ...
                if bv_int == value_int:
                    arg_str.append('t'+str(key))
                    bv_find = True
                    break
            
            if not bv_find:
                for i in range(1000):
                    if i not in inst.state.scratch.temps:
                        arg_str.append('t'+str(i))
                        inst.state.scratch.temps[i] = bv_value
                        break
        
        stmt_str = 'PUT(offset=8) = func_'+func_name+'('
        for i in range(argc_ir):
            stmt_str += arg_str[i]+','

        stmt_str = stmt_str[0:-1]+')'
        
        out_str = stmt_str+'--->'
        
        for i in range(argc_ir):
            out_str += arg_str[i]+':'+str(argv_ir[i])+';'
        
        if func_name == 'snprintf' or func_name == 'sprintf' or func_name == 'strchr' or func_name == 'strlen':
            cur_index = 0
            
            while cur_index <= 256:
                cur_int8_str = str(inst.state.mem[argv_ir[0]+cur_index].int8_t).split(' ')[2][0:-1]
                cur_int8_value = None
                if '0x' in cur_int8_str:
                    cur_int8_value = int(cur_int8_str,16)
                else:
                    cur_int8_value = int(cur_int8_str)
            ...
            out_str += 'len:'+str(cur_index)+';'
            
        elif func_name == 'strcat' or func_name == 'strcmp' or func_name == 'strcpy' or func_name == 'strstr':
            cur_index = 0
            ...
            out_str += 'len1:'+str(cur_index)+';'
            
            #
            cur_index = 0
            while cur_index <= 256:
                cur_int8_str = str(inst.state.mem[argv_ir[1]+cur_index].int8_t).split(' ')[2][0:-1]
                cur_int8_value = None
                if '0x' in cur_int8_str:
                    cur_int8_value = int(cur_int8_str,16)
                else:
    ...
    #my add
    def add_function_ir(self,inst):
        
        
        func_ir = ''
        func_name = None
        func_name = str(inst).split(' ')[1]
        if '>' in func_name:
            func_name = func_name[0:-1]
        
        func_ir = self.handle_func(inst,func_name)
    ...
    def execute(self, state, successors=None, arguments=None, ret_to=None):
        ...
        if callable(override):
            ...
            #my add
            function_ir = self.add_function_ir(inst)
            if function_ir != '':
                state.ir_trace.append(function_ir)
